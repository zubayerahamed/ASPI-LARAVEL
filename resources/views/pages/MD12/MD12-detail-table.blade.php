@if (isset($productOptionDetail) && $productOptionDetail != null && $productOptionDetail->product_option_id != 'RESET')
    <div class="card card-default">
        ...
        <div class="card-body">
            <form id="detailform" action="{{ $productOptionDetail->id == null ? route('MD12.detail-table.create') : route('MD12.detail-table.update', ['id' => $productOptionDetail->id]) }}" method="POST">
                @csrf
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <div class="flex-grow-1 text-left">
                        <button
                                ...
                                data-reloadurl="{{ route('MD12.detail-table', ['id' => 'RESET', 'product_option_id' => $productOptionDetail->product_option_id]) }}"
                                type="reset"
                    ...
                    <div class="flex-grow-1 justify-content-end d-flex gap-2">
                        ...
                        @else
                            <button data-url="{{ route('MD12.detail-table.delete', ['id' => $productOptionDetail->id]) }}" type="button" class="btn btn-sm btn-danger btn-detail-delete d-flex align-items-center gap-2">
        ...
        @if (!$detailList->isEmpty())
            <div class="table-responsive data-table-responsive">
                <table class="table table-hover table-bordered p-0 m-0 MD12-datatable-fragment">
                    <thead>
                        <tr>
                            <th>Label</th>
                    ...
                    <tbody>
                        @foreach ($detailList as $x)
                            <tr>
                                <td>
                                    <a data-reloadurl="{{ route('MD12.detail-table', ['id' => $x->id, 'product_option_id' => $x->product_option_id]) }}" class="detail-dataindex" data-reloadid="detail-table-container" href="#">{{ $x->label }}</a>
                                </td>
                                <td>{{ $x->additional_price }}</td>
                                <td>{{ ucfirst($x->price_type) }}</td>
                                <td class="text-center">{{ $x->seqn }}</td>
                                <td>
                                    <div class="d-flex justify-content-end align-items-center gap-2">
                                        <button data-url="{{ route('MD12.detail-table.delete', ['id' => $x->id]) }}" type="button" class="btn btn-sm btn-danger btn-table-delete d-flex align-items-center">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('.MD12-datatable-fragment').on('click', 'a.detail-dataindex', function(e) {
                e.preventDefault();
            ...
            $('.MD12-datatable-fragment').on('click', 'button.btn-table-delete', function(e) {
                e.preventDefault();
                sweetAlertConfirm(() => {
                    deleteRequest($(this).data('url'));
                });
